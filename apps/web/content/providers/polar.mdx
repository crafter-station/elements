...
const client = new polar.Polar({
  accessToken: process.env.POLAR_ACCESS_TOKEN || "", // [!code highlight]
  server: "sandbox",
});
...
export async function GET(request: Request) {
  ...
  const checkout = await client.checkouts.custom.create({ // [!code focus]
    productPriceIds: products, // [!code focus]
    successUrl: `${request.url.split("/api")[0]}/success`, // [!code focus]
  }); // [!code focus]
